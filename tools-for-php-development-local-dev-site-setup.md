...
intro: What PHP development tools are available? What are the pros and cons of each tool?
...
In essence, a local development environment comprises **everything needed to _run_ local dev sites**. This commonly includes software that needs to be installed and configured to work together in concert, such as a web server, database, and so forth.
